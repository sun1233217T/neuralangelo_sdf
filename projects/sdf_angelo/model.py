...
class Model(BaseModel):
    ...
    def render_ray_surface(self, center, ray_unit, sample_idx=None, stratified=False, sample_eps=1e-3):
        ...
        # Prepare a differentiable surface point tensor.
        surf_pts_full = torch.zeros_like(center)
        surf_pts_full[valid] = surf_pts[valid]
        surf_pts_full = surf_pts_full.detach().requires_grad_(True)
        ...
        sdf_surface = torch.zeros(*center.shape[:2], device=center.device, dtype=center.dtype)
        if valid.any():
            pts_valid = surf_pts_full[valid]  # [Nh,3] leaf with grad
            ...
            # Scatter back with mask.
            mask3 = valid.unsqueeze(-1).expand_as(rgbs)  # [B,R,3]
            rgbs = rgbs.masked_scatter(mask3, rgbs_v)
            gradients = gradients.masked_scatter(mask3, grads_v)
            if hessians is not None and hess_v is not None:
                hessians = hessians.masked_scatter(mask3, hess_v)
            sdf_surface = sdf_surface.masked_scatter(valid, sdfs_v.squeeze(-1))
        ...
        output = dict(
            ...
            surf_pts=surf_pts_full,  # [B,R,3], requires_grad for color-shift supervision
            hit_mask=valid,  # [B,R]
            depth = depth, # [B,R]
            outside=outside,  # [B,R]
            # t_vals=t_vals,  # [B,R]
            gradients=gradients,  # [B,R,3] for eikonal / normals
            gradient = gradients,  # alias
            hessians=hessians,  # [B,R,3]/None
            sdf_surface=sdf_surface,  # [B,R]
        )
        return output 
