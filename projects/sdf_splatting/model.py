...
class Model(BaseModel):
    ...
    def __init__(self, cfg_model, cfg_data):
        ...
        self.train_with_local_shift_loss = cfg_data.train.local_shift_load
        self.train_with_local_shift_num = 4 + 1
        # Define models.
        ...
        # Surface sampling/refinement controls.
        self._init_surface_refine(cfg_model)
    ...
    def inference(self, data):
        ...
        output = self.render_image(data["pose"], data["intr"], image_size=self.image_size_val,
                                   stratified=False, sample_idx=data["idx"], train = False)  # [B,N,C]
    ...
    def render_image(self, pose, intr, image_size, stratified=False, sample_idx=None, train = False):
        ...
        for center, ray, _ in self.ray_generator(pose, intr, image_size, full_image=True):
            ...
            output_batch = self.render_rays(center, ray_unit, sample_idx=sample_idx, stratified=stratified, train = train)
    ...
    def render_pixels(self, pose, intr, image_size, stratified=False, sample_idx=None, ray_idx=None, train = True):
        ...
        ray_unit = torch_F.normalize(ray, dim=-1)  # [B,R,3]
        output = self.render_rays(center, ray_unit, sample_idx=sample_idx, stratified=stratified, train = train)
        return output

    def render_rays(self, center, ray_unit, sample_idx=None, stratified=False, train = False):
        with torch.no_grad():
        ...
        output_object = self.render_rays_object(center, ray_unit, near, far, outside, app, stratified=stratified, train = train)
        ...
        surface_sdf = output_object["surface_sdf"]  # [B,R]
        surface_rgb = output_object["surface_rgb"] * output_object["hit_mask"][..., None].float()  # [B,R,3]
        ...
        output = dict(
            ...
            surface_sdf=surface_sdf,  # [B,R]
            surface_rgb=surface_rgb,  # [B,R,3]
            surface_depth=output_object["surface_depth"],  # [B,R,1]
            surface_normal=output_object["surface_normal"],  # [B,R,3]
        )
        if output_object.get("surface_shift_target", None) is not None:
            output["surface_shift_target"] = output_object["surface_shift_target"] # [B,R,3]
        return output

    def render_rays_object(self, center, ray_unit, near, far, outside, app, stratified=False, surf_p_sample_n=3, surf_p_sample_eps=1e-3, train = False):
        # Base sampling along each ray to estimate surface location.
        with torch.no_grad():
        ...
        if self.surface_samples_enabled:
            ...
            # Densify samples around the estimated surface for sharper surface outputs.
            with torch.no_grad():
                dists, surface_idx = self.sample_dists_all(center, ray_unit, near, far, stratified=stratified,
                                                          surf_pts_t=surface["t_vals"],
                                                          hit_mask=surface["hit_mask"],
                                                          surf_p_sample_n=surf_p_sample_n,
                                                          surf_p_sample_eps=surf_p_sample_eps)  # [B,R,N,1]
                if self.train_with_local_shift_loss and train:
                    # 仅在训练时启用 local shift loss，偏移点沿 base ray 计算。
                    num_shift = self.train_with_local_shift_num
                    t_vals = surface["t_vals"]  # [B,R]
                    B, R = t_vals.shape
                    t_group = t_vals.view(B, -1, num_shift)  # [B,G,K]
                    center_group = center.view(B, -1, num_shift, 3)
                    ray_group = ray_unit.view(B, -1, num_shift, 3)
                    base_center = center_group[:, :, :1, :]
                    base_ray = ray_group[:, :, :1, :]
                    local_shift_target = (base_center + base_ray * t_group[..., None]).view(B, R, 3)
                    surface["local_shift_target"] = local_shift_target  # [B,R,3]
        ...
        output = dict(
            ...
            hit_mask=surface["hit_mask"],  # [B,R]
            surface_rgb=surface["surface_rgb"],  # [B,R,3]
            surface_sdf=surface["surface_sdf"],  # [B,R]
            surface_normal=surface["surface_normal"],  # [B,R,3]
            surface_depth=surface["surface_depth"],  # [B,R,1]

            surface_shift_target=surface.get("local_shift_target", None),  # [B,R,3]/None
        )
        return output
    ...
    def sample_dists_all(
        ...
        if surf_pts_t is not None:
            ...
            if hit_mask is not None:
                d_fallback = dists[..., :1, :].expand_as(d_ext)
                d_ext = torch.where(hit_mask[..., None, None], d_ext, d_fallback)
