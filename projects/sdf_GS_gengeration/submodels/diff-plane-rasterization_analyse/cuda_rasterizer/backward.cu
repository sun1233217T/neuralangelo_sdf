...
renderCUDA(
	...
	const float* __restrict__ dl_dH_H,
	const float* __restrict__ dl_dpixopa,
	...
	const bool render_geo)
{
	...
	__shared__ float collected_dl_dH_H[BLOCK_SIZE];
	__shared__ float dL_dH_H_rec[BLOCK_SIZE];
	...
	if (inside) {
		...
		collected_dl_dpixopa = dl_dpixopa[pix_id];
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			// 新增：加载预先计算的 dl_dH_H
			collected_dl_dH_H[block.thread_rank()] = dl_dH_H[coll_id];
		}
		block.sync();

		// 在进入反向传播的内层循环前，定义一个累加器，用于累计已处理的 dl_dH_H（对应原始顺序中索引大于当前核的部分）
		float accumulated_dl_dH_H = 0.0f;
		float total_dl_dH_H = 0.0f;
		int num_gaussians = min(BLOCK_SIZE, toDo); // 当前块内的高斯核数
		for (int j = 0; j < num_gaussians; j++) {
			total_dl_dH_H += collected_dl_dH_H[j];
		}
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// ------------- 新增梯度更新 -------------
			// 对于可以进行表观不透明度梯度计算的情况：
			// if (collected_dl_dH_H[j] > 1e-6){
			// 对于当前核本身（相当于 n 中的情况），加上： dl_dH_H / alpha
			dL_dalpha += collected_dl_dH_H[j] / alpha;
			
			// 对于当前核 i，需要累计未来（原始顺序中 n<i）的贡献
			// 未来核 dl_dH_H 的总和 = total_dl_dH_H - (accumulated_dl_dH_H + collected_dl_dH_H[j])
			dL_dalpha += (total_dl_dH_H - (accumulated_dl_dH_H + collected_dl_dH_H[j])) / (1.f - alpha);
			// -----------------------------------------

			dL_dalpha /= T; // 除以 T，因为之后有再乘以 T

			// dL_dalpha += collected_dl_dpixopa / num_gaussians; //来自图片像素不透明度的梯度
			accum_alpha_rec = last_alpha + (1.f - last_alpha) * accum_alpha_rec;
			dL_dalpha += (1 - accum_alpha_rec) * collected_dl_dpixopa;  //- (alpha - accum_alpha_rec) * dL_dalpha;
			...
			// 更新累计值：将当前核的 dl_dH_H 加入累积，用于后续当前核的梯度更新
			accumulated_dl_dH_H += collected_dl_dH_H[j];
		}
	}
}
