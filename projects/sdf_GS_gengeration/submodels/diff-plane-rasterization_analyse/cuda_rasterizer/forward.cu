...
renderCUDA(
	...
	float* app_opacity)
{
	auto atomicMaxFloat = [](float* addr, float value) {
        int* addr_as_i = reinterpret_cast<int*>(addr);
        int old = *addr_as_i;
        int expected;
        do {
            expected = old;
            if (__int_as_float(expected) >= value)
                break;
            old = atomicCAS(addr_as_i, expected, __float_as_int(value));
        } while (old != expected);
    };
	...
	__shared__ float collected_app_opacity[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_app_opacity[block.thread_rank()] = app_opacity[coll_id];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			collected_app_opacity[j] = max(collected_app_opacity[j], T * alpha); //小危险的写入表面alpha
			if (test_T < 0.0001f)
			{
				done = true;
				continue;
		...
		block.sync(); // 确保所有线程完成对共享内存的修改

        // 每个线程负责写回自己加载的数据点
        int o_progress = i * BLOCK_SIZE + block.thread_rank();
        if (range.x + o_progress < range.y)
        {
            int coll_id = collected_id[block.thread_rank()];
            float val = collected_app_opacity[block.thread_rank()];
            atomicMaxFloat(&app_opacity[coll_id], val);
        }
