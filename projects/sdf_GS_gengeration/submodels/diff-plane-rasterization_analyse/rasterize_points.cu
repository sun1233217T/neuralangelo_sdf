	...
	const bool debug)
{
  ...
  torch::Tensor app_opacity = torch::full({P, 1}, 1.0, means3D.options().dtype(torch::kFloat32)).contiguous();
  float* app_opacityptr = nullptr;
  app_opacityptr = app_opacity.data<float>();

  torch::Tensor out_alpha = torch::full({1, H, W}, 1.0, means3D.options().dtype(torch::kFloat32)).contiguous();
  float* out_alpha_ptr = nullptr;
  out_alpha_ptr = out_alpha.data<float>();
  ...
  return std::make_tuple(rendered, out_color, radii, out_observe, out_all_map, out_plane_depth, geomBuffer, binningBuffer, imgBuffer, app_opacity, out_alpha);
}
...
RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dl_dH_H,
	const torch::Tensor& dl_dpixopa,
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos,
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dl_dH_H.contiguous().data<float>(),
	  dl_dpixopa.contiguous().data<float>(),
